...
var demofunction = edge.func({
  source:function() {/*
      ...
      public class Startup : DPFP.Capture.EventHandler{
        ...
        {
            // MessageBox.Show(System.IO.Directory.GetCurrentDirectory(), "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
...
const clockType = args[1]

demofunction({ username, clockType }, (err, result) => {
    if (err) throw err
    ...
    if (result === 'SUCCESS') {
        if (clockType === 'CLOCK_IN') {
            ...
            url = `http:127.0.0.1:8000/api/v1/attendance/${username}`
            axios.get(url)
            .then(res => {
                // console.log(`http:127.0.0.1:8000/api/v1/attendance/${res.data.id}`)
                axios.put(`http:127.0.0.1:8000/api/v1/attendance/${res.data.data.id}`, {
                    nik: username,
                    clockType: clockType
                })
                .then(res => {
